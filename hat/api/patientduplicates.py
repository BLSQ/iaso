    ...
    * csv: request an answer in a CSV file for mass verification. The CSV is always including the full information.
    ...
    def list(self, request):
        ...
        if not (
            ...
            if search_postname:
                queryset = queryset.filter(
                    Q(patient1__post_name__contains=search_postname)
                    | Q(patient2__post_name__contains=search_postname)
                )
            if search_prename:
                queryset = queryset.filter(
                    Q(patient1__first_name__contains=search_prename)
                    | Q(patient2__first_name__contains=search_prename)
                )
            if search_lastname:
                queryset = queryset.filter(
                    Q(patient1__last_name__contains=search_lastname)
                    | Q(patient2__last_name__contains=search_lastname)
                )
        ...
        if request.user.profile.province_scope.count() != 0:
            user_province_list = get_user_geo_list(
                request.user, "province_scope"
            ).distinct()
            queryset = queryset.filter(
                Q(patient1__origin_area__ZS__province_id__in=user_province_list)
        ...
        if request.user.profile.ZS_scope.count() != 0:
            ...
            queryset = queryset.filter(
                Q(patient1__origin_area__ZS_id__in=user_zs_list)
        ...
        if coordination_id or teams:
            ...
            queryset = queryset.annotate(teams_cases=Exists(team_cases)).filter(
                teams_cases=True
            )
        ...
        if confirmation_result is not None:
            if confirmation_result == "not_done":
                ...
                confirmed_cases = CaseView.objects.filter(confirmed_case=True).filter(
                    Q(normalized_patient_id=OuterRef("patient1_id"))
                    | Q(normalized_patient_id=OuterRef("patient2_id"))
                )
                queryset = queryset.annotate(
        ...
        if test_types:
            test_with_type_in = Test.objects.filter(
                Q(form__normalized_patient_id=OuterRef("patient1_id"))
        ...
        if patientId1 and patientId2:
            queryset = queryset.filter(
                (Q(patient1_id=patientId1) & Q(patient2_id=patientId2))
        ...
        if csv_format is None and xlsx_format is None:
            ...
            res["patientduplicatepairs"] = list(
                map(lambda x: x.as_dict(full), page.object_list)
            )
            ...
            def get_row(dupe, **kwargs):
                return [
                    ...
                    if dupe["patient1__origin_area__name"]
                    else dupe["patient1__origin_raw_AS"],
                    dupe["patient1__origin_village__name"]
                    if dupe["patient1__origin_village__name"]
                    else dupe["patient1__origin_raw_village"],
                    dupe["patient2_id"],
            ...
            if csv_format:
                response = StreamingHttpResponse(
                    streaming_content=(
                        iter_items(limited_queryset, Echo(), columns, get_row)
                    ),
        ...
        if is_authorized:
            ...
            if merge:
                ...
                if merge == patient_dupe.patient1_id:
                    ...
                    return Response(
                        "merge field should be the ID of either patient 1 or patient 2",
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                result = merge_patient_duplicate(
                    patient_dupe, merge_from, merge_to, request.user
                )
    ...
    def create(self, request):
        ...
        if patientA and patientB:
            ...
            if is_authorized:
                ...
                log_modification(
                    None, newPatientDuplicatesPair, PATIENT_API, request.user
                )
