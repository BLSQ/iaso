...
class OrgUnitTypeViewSetV2(ModelViewSet):
    ...
    def dropdown(self, request, *args):
        ...
        version_id = SourceVersionIdSerializer(data=self.request.query_params).get_source_version_id(raise_exception=True)
        if version_id:
            type_ids_in_version = OrgUnit.objects.filter(version_id=version_id).distinct("org_unit_type_id").values_list("org_unit_type_id", flat=True)
