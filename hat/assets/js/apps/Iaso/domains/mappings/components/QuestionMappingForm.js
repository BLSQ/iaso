        ...
        options.forEach((program) => {
            program.programTrackedEntityAttributes.forEach(
                (trackedEntityTypeAttributeRaw) => {
                    const trackeEntityAttribute = trackedEntityTypeAttributeRaw.trackedEntityAttribute;
                    if (
                        trackeEntityAttribute.name.toLowerCase().includes(token)
            || (trackeEntityAttribute.code
              && trackeEntityAttribute.code.toLowerCase().includes(token))
                    ) {
    ...
    const mapToMappingProgramElements = (options, input) => {
        ...
        options.forEach((program) => {
            program.programStages.forEach((programStage) => {
                programStage.programStageDataElements.forEach((programStageElement) => {
                    const { dataElement } = programStageElement;
                    if (
                        dataElement.name.toLowerCase().includes(token)
            || (dataElement.code && dataElement.code.toLowerCase().includes(token))
                    ) {
                        dataElement.categoryCombo.categoryOptionCombos.forEach((coc) => {
                            if (mapping.mapping.mapping_type === 'EVENT_TRACKER') {
                                results.push({
...
const QuestionMappingForm = ({
    ...
    const withinRepeatGroup = Descriptor.withinRepeatGroup(question, indexedQuestions);
    ...
    const programId = repeatGroupMapping ? repeatGroupMapping[0].program_id : mapping.derivate_settings.program_id

    if (question.type =="repeat") {
      return <EventTrackerProgramForm dataSourceId={mapping.mapping.data_source.id} repeatGroupMapping={repeatGroupMapping} onConfirmedQuestionMapping={onConfirmedQuestionMapping}></EventTrackerProgramForm>
    }
