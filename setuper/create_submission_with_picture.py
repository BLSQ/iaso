...
def define_health_facility_reference_form(iaso_client):
    org_unit_types = iaso_client.get(
        "/api/v2/orgunittypes/?fields=id,name,units_count,projects,sub_unit_types,allow_creating_sub_unit_types&search=Health facility/Formation sanitaire - HF"
    )["results"]
    ...
    iaso_client.patch(f"/api/v2/orgunittypes/{health_facility_type['id']}/", json=health_facility_type)
    update_reference_forms = iaso_client.get(f"/api/v2/orgunittypes/{health_facility_type['id']}/")
