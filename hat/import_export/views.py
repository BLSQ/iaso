...
def upload_cases(request: HttpRequest) -> HttpResponse:
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # run import task
            task = run_task(import_task, args=[fileinfos], permission="cases.import")
            return redirect("datasets:import_cases:state", task_id=task.id)
    else:
        form = UploadFileForm(True, ".mdb,.accdb,.enc")
    return render(
        request,
        "import_export/upload_cases.html",
        {
            "form": form,
            "menu": get_menu(request.user, reverse("datasets:import_cases:upload")),
        },
    )
...
def upload_locations(request: HttpRequest) -> HttpResponse:
    ...
    return render(
        request,
        "import_export/upload_locations.html",
        {
            "form": form,
            "menu": get_menu(request.user, reverse("datasets:import_locations:upload")),
        },
    )
