...
class TeamAPITestCase(APITestCase):
    ...
    def test_query_ancestor(self):
        ...
        response = self.client.get(f"/api/microplanning/teams/?ancestor_id={team_a.id}", format="json")
        r = self.assertJSONResponse(response, 200)
        self.assertEqual(len(r), 0)
        response = self.client.get(f"/api/microplanning/teams/?ancestor_id={team_b.id}", format="json")
        r = self.assertJSONResponse(response, 200)
        self.assertEqual(len(r), 4)
        ids = sorted([row["id"] for row in r])
        self.assertEqual(ids, [team_b_c.id, team_b_d.id, team_b_c_e.id, team_b_c_f.id])
        response = self.client.get(f"/api/microplanning/teams/?ancestor_id={team_b.id}&search=hello", format="json")
        r = self.assertJSONResponse(response, 200)
        self.assertEqual(len(r), 1)
        ids = sorted([row["id"] for row in r])
        self.assertEqual(ids, [team_b_c_f.id])
        response = self.client.get(f"/api/microplanning/teams/?ancestor_id={team_b_c.id}", format="json")
        r = self.assertJSONResponse(response, 200)
        self.assertEqual(len(r), 2)
        ids = sorted([row["id"] for row in r])
