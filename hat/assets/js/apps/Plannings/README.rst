...
This tool is strongly inspired by the existing `QGIS tool <http://www.qgis.org>`__.
...
#. The planner selects villages which have had a confirmed case of HAT in the
   ...
   **Dashboard**: The tool allows to “highlight” villages with HAT cases in
   certain years. The “Select villages around confirmed HAT cases” button
   implements a shortcut to select all those villages in one time.
...
#. On the QGIS map, the number of cases in a particular location is indicated by
   ...
   **Dashboard**: The size of the dot depends on the village type, the colour
...
#. The planner adds a buffer to the above villages to identify which neighbouring
   ...
   **Dashboard**: The “highlight buffer zone” is drawn when the “Select villages”
   option is enabled.


#. The buffer zone is configurable between 1 and 5 km in radius.

   **Dashboard**: There is no limit on the buffer zone size and can be
   manually adjusted.
...
#. The planner may then choose to add in some additional neighbouring villages
   ...
   **Dashboard**: Instead of drawing a polygon the tool draws a dynamic buffer
   zone that follows the mouse movements. Clicking on it select/deselect the
   villages within the buffer zone.


#. The planner exports a list of all villages selected.

   **Dashboard**: The list is exported in Excel format.
...
#. The planner uses this list to plan a route for the data collectors, taking
   ...
   **Dashboard**: Every time a new village is added to the list the number of
   ...
   participants.
   The population estimate is uploaded within the location dbf files.
...
- The open source software solution has a lot of options and requires
  ...
  **Dashboard**: Simplifies user interface.
...
- Simultaneous visibility of multiple zones - occasionally, a village near to
  ...
  **Dashboard**: Supports cross-zonal planning.
...
- Selections are temporary - as the application is desktop-based it is hard
  ...
  **Dashboard**: This is still an issue but it is also possible to print or to
  export as a pdf file the current map view.
...
The necessary datasets are:
  ...
  * The province, zone and aire de santé boundaries, condensed in
    the `topojson <https://github.com/topojson/topojson>`__ file:
...
into one `topojson <https://github.com/topojson/topojson>`__ file with this schema:
...
Map tiles:
  ...
  - `ArcGIS <http://www.arcgis.com>`__

Map tools:
  - `Leaflet <http://leafletjs.com/>`_
  - `topojson <https://github.com/topojson/topojson>`__
  - `MapShaper viewer <http://mapshaper.org>`__

API Requests:
  - Locations: :func:`hat.api.dataset_viewset.locations_with_shape`
  - Villages with HAT cases: :func:`hat.api.dataset_viewset.data_by_location`
