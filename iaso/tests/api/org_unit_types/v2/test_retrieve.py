...
class OrgUnitTypesV2RetrieveTestCase(APITestCase, SwaggerTestCaseMixin):
    ...
    def assertValidData(self, res_data):
        self.assertResponseCompliantToSwagger(res_data, "OrgUnitTypeRetrieve")

    def test_permissions(self):
        res = self.client.get(reverse("orgunittypes_v2-detail", kwargs={"pk": self.org_unit_type_1.pk}))
        ...
        res = self.client.get(reverse("orgunittypes_v2-detail", kwargs={"pk": self.org_unit_type_6.pk}))
        self.assertJSONResponse(res, status.HTTP_404_NOT_FOUND)
    ...
    def test_num_queries(self):
        ...
        with self.assertNumQueries(5):
            res = self.client.get(reverse("orgunittypes_v2-detail", kwargs={"pk": self.org_unit_type_3.pk}))
