...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        no_path_count = OrgUnit.objects.filter(path=None).count()
        self.stdout.write(f"Found {no_path_count} without path")

        root_org_units = OrgUnit.objects.filter(parent=None)
        self.stdout.write(f"Found {len(root_org_units)} root org units")
        ...
        if no_path_count == 0:
            ...
            self.stdout.write(f"After this operation, {no_path_count} org units are left without path")
    ...
    def _create_org_unit_batch(self, level, parent=None):  # TODO: remove me
        ...
        for _ in range(10):
            new_parent = OrgUnit.objects.create(parent=parent, name="set_org_units_path test")
            created += 1
