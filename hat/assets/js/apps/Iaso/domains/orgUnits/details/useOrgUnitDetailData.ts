...
// Kept as its own module (not in the shared `orgUnits/hooks.ts`) so the org unit types fix below
// only ever touches this page's own file, not the widely-reused `useSaveOrgUnit`/`useRefreshOrgUnit`
// exports that live alongside `useOrgUnitDetailData` there.
export const useOrgUnitDetailData = (
    ...
    const onSelectOrgUnitTypes = useCallback(
        (data: OrgunitTypes): OrgUnitTypeDropdownOption[] => {
            const orgUnitTypes =
                data?.map((ot, i) => ({
                    ...ot,
    ...
    >([
        ...
        {
            // v2 dropdown is much faster than the plain list (no nested serialization)
            queryKey: ['orgUnitTypesDropdown'],
