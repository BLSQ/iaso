...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for province in Province.objects.order_by("name"):
            zones = []
            province_dict = {"id": str(province.id), "name": province.name}

            # print("PROVINCE", province.name)
            for zone in province.zs_set.order_by("name"):
                areas = []
                zone_dict = {"id": str(zone.id), "name": zone.name}
                print("ZONE", zone.name)

                for area in zone.as_set.order_by("name"):
                    villages = []
                    structures = []
                    area_dict = {"id": str(area.id), "name": area.name}
                    # print("area", area.name)

                    for village in area.village_set.filter(
                        Q(village_official="YES") | Q(village_source="device")
                    ).order_by("name"):
                        village_dict = {
                            "id": str(village.id),
                            "name": village.name,
