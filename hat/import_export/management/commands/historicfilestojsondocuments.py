    ...
    def handle(self, *args, **options):
        ...
        for event in all_events:
            if event["sub_type"] == options["type"] or options["type"] == "all":
                ...
                event_details = (
                    get_event_of_type(event["table_name"], event["id"])
                ...
                hash_already_imported = JSONDocument.objects.filter(
                    doc_id__istartswith=f"{file_hash}-"
                ).first()
                tables = {"historic": "T_CARDS", "pv": "tblFishedeDeclaration"}
                if (
                    event["sub_type"] in ["historic", "pv"]
                    and hash_already_imported is None
                ):
                    save_documents(
                        prepared.get(tables[event["sub_type"]]),
                        file_hash,
                        type=event["sub_type"],
                    )
                    print(
                        "Transform documents to try and find their case...",
                        end="",
                        flush=True,
                    )
                    ...
                    for index, row in transformed.iterrows():
                        ...
                        ).filter(source=event["sub_type"])
                        if cases.count() == 1:
                            JSONDocument.objects.filter(
                                id=row["json_document_id"]
                            ).update(case_id=cases.first().id)
