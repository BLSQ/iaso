...
survey_data = {
    'type': [
        'text', 'integer', 'date', 'decimal', 'select_one yes_no',
        'select_multiple symptom_list', 'text',
        'integer', 'decimal'  # For complex constraint test
    ],
    'name': [
        'respondent_id', 'age', 'visit_date', 'temperature', 'consent',
        'symptoms', 'feedback',
        'integer_check', 'decimal_check'  # For complex constraint test
    ],
    'label': [
        'Respondent ID (e.g., ABC123)', 'Age (Years)', 'Date of Visit', 'Body Temperature (Celsius)', 'Consent Given?',
        'Select Symptoms Experienced (Multiple)', 'Optional Feedback',
        'Integer Check', 'Decimal Check'  # For complex constraint test
    ],
    'required': [
...
choices_data = {
    'list_name': [
        ...
        'symptom_list', 'symptom_list', 'symptom_list', 'symptom_list'
    ],
    'name': [
        'yes', 'no', 'dk',
...
data_rows = [
    # Row 1: Should Pass
    {'respondent_id': 'XYZ789', 'age': 35, 'visit_date': today_str, 'temperature': 37.2, 'consent': 'yes', 'symptoms': 'fever cough', 'feedback': 'Feeling better.', 'integer_check': 5, 'decimal_check': 6.1},
    # Row 2: Fail (Required field 'age' is empty)
    {'respondent_id': 'ABC111', 'age': None, 'visit_date': today_str, 'temperature': 38.0, 'consent': 'yes', 'symptoms': 'cough', 'feedback': '', 'integer_check': 10, 'decimal_check': 10.1},
    # Row 3: Fail (Type error: 'age' is text)
    {'respondent_id': 'DEF222', 'age': 'Twenty Five', 'visit_date': today_str, 'temperature': 36.8, 'consent': 'no', 'symptoms': '', 'feedback': 'Typo test.', 'integer_check': 2, 'decimal_check': 3},
    # Row 4: Fail (Constraint: 'age' < 19)
    {'respondent_id': 'GHI333', 'age': 15, 'visit_date': yesterday_str, 'temperature': 37.0, 'consent': 'yes', 'symptoms': 'headache', 'feedback': '', 'integer_check': 8, 'decimal_check': 8.8},
     # Row 5: Fail (Constraint: 'temperature' too high)
    {'respondent_id': 'JKL444', 'age': 45, 'visit_date': today_str, 'temperature': 44.1, 'consent': 'yes', 'symptoms': 'fever', 'feedback': '', 'integer_check': 1, 'decimal_check': 5},
    # Row 6: Fail (Constraint: 'respondent_id' regex mismatch)
    {'respondent_id': 'MNO-555', 'age': 60, 'visit_date': today_str, 'temperature': 36.5, 'consent': 'yes', 'symptoms': 'fatigue', 'feedback': 'ID format wrong.', 'integer_check': 15, 'decimal_check': 25},
    # Row 7: Fail (select_one: 'consent' value 'maybe' not in choices)
    {'respondent_id': 'PQR666', 'age': 28, 'visit_date': today_str, 'temperature': 37.1, 'consent': 'maybe', 'symptoms': 'cough', 'feedback': '', 'integer_check': 20, 'decimal_check': 20.5},
    # Row 8: Fail (select_multiple: 'symptoms' includes 'nausea' which is not in choices)
    {'respondent_id': 'STU777', 'age': 52, 'visit_date': yesterday_str, 'temperature': 38.5, 'consent': 'yes', 'symptoms': 'fever nausea cough', 'feedback': 'Invalid symptom.', 'integer_check': 3, 'decimal_check': 4},
    # Row 9: Pass (Required field 'consent' has valid 'dk' value, optional fields empty)
    {'respondent_id': 'VWX888', 'age': 70, 'visit_date': today_str, 'temperature': None, 'consent': 'dk', 'symptoms': '', 'feedback': '', 'integer_check': 7, 'decimal_check': 7.7},
    # Row 10: Pass (Boundary age, valid multiple symptoms)
    {'respondent_id': 'YZA999', 'age': 19, 'visit_date': today_str, 'temperature': 36.9, 'consent': 'yes', 'symptoms': 'fatigue headache', 'feedback': 'Boundary test.', 'integer_check': 9, 'decimal_check': 19},
    # Row 11: Fail (Date constraint: Future date - script might skip this check)
    {'respondent_id': 'BCD000', 'age': 33, 'visit_date': tomorrow_str, 'temperature': 37.0, 'consent': 'yes', 'symptoms': '', 'feedback': 'Future date.', 'integer_check': 11, 'decimal_check': 11.1},
    # Row 12: Skipped Constraint (decimal_check <= integer_check, but script skips ${} references)
    {'respondent_id': 'EFG111', 'age': 41, 'visit_date': today_str, 'temperature': 37.0, 'consent': 'yes', 'symptoms': 'cough', 'feedback': 'Constraint check', 'integer_check': 10, 'decimal_check': 5.5},
