...
class MissionAPIListTestCase(SwaggerTestCaseMixin, APITestCase):
    ...
    def test_filters(self):
        ...
        with self.subTest("Mission type filter"):
            ...
            self.assertCountEqual(
                [x["id"] for x in res_data["results"]], [self.mission_out_1.pk, self.mission_out_2.pk]
            )

            res = self.client.get(reverse("missions-list"), data={"mission_type": MissionType.ENTITY_AND_FORM})
            res_data = self.assertJSONResponse(res, status.HTTP_200_OK)
            self.assertValidData(res_data, 2)
