...
class SetupAccountApiTestCase(APITestCase):
    ...
    def test_setup_account_unauthorized(self):
        self.client.force_authenticate(self.user)
        response = self.client.post("/api/setup_account/", data={}, format="json")
        self.assertEqual(response.status_code, 403)

    def test_setup_account_duplicate_account(self):
        ...
        response = self.client.post("/api/setup_account/", data=data, format="json")

        self.assertEqual(response.status_code, 400)
        j = response.json()
        self.assertIn("account_name", j)

    def test_setup_account_duplicate_datasource(self):
        self.client.force_authenticate(self.admin)
        data = {
            "account_name": "Korogu",
            "user_username": "unittest_username",
    ...
    def test_setup_account_duplicate_user(self):
        self.client.force_authenticate(self.admin)
        data = {
    ...
    def test_setup_account_create(self):
        self.client.force_authenticate(self.admin)
        data = {
            "account_name": "unittest_account",
            "user_username": "unittest_username",
