...
class ProfileAPITestCase(APITestCase):
    ...
    def test_search_user_by_has_email(self):
        ...
        response = self.client.get(reverse("profiles-list", kwargs={"version": "v2"}), {"has_email": True})
        response_data = self.assertJSONResponse(response, 200)
        self.assertEqual(len(response_data["results"]), 0)
        self.assertTrue(all(x["email"] for x in response_data["results"]))

        response = self.client.get(reverse("profiles-list", kwargs={"version": "v2"}), {"has_email": False})
        response_data = self.assertJSONResponse(response, 200)
        self.assertEqual(len(response_data["results"]), 7)
    ...
    def test_update_user_add_phone_number(self):
        ...
        self.assertEqual(response_data["phone_number"], "+32477123456")
    ...
    def test_list_profiles_sorted_by_annotated_first_user_role(self):
        ...
        response = self.client.get(
            reverse("profiles-list", kwargs={"version": "v2"}), {"order": "annotated_first_user_role", "limit": 10}
        )
        response_data = self.assertJSONResponse(response, 200)
