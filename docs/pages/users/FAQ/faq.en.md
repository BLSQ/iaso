...
### How do I configure Iaso in a way that mobile users cannot create new OUs? And/or is it possible to limit OU creation to certain OU types? Ex: mobile users can create the types at the bottom of the hierarchy (FOSA, village), but not at the top (Region -> Aire sanitaire)?
...
1. It can be scheduled as periodic task, which means it is set to run regularly.
...
2. followup: the successive visits when the beneficiary is already admitted into program
...
In the context of nutrition program, a visit is split into 3 steps: anthropometric, medical and assistance.
