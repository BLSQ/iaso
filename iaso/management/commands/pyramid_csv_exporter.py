    ...
    def handle(self, *args, **options):
        ...
        with open(file_name, 'w') as units_file:
            ...
            for t in OrgUnit.objects.filter(version=version).prefetch_related('org_unit_type').order_by('id'):
                writer.writerow((t.source_ref if t.source_ref else "iaso_%d" % t.id,
                                 t.name,
                                 None if t.parent is None else(t.parent.source_ref if t.parent.source_ref else "iaso_%d" % t.parent.id),
                                 '',
