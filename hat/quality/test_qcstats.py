...
def compare_dict(test_instance, expected, result, message_prefix=""):
    test_instance.assertTrue(len(expected) >= len(result),
                             f"{message_prefix}dict length mismatch, expected {len(expected)}, got {len(result)}")
    for k, v in expected.items():
        test_instance.assertEquals(v, result.get(k),
                                   f"{message_prefix}key {k} mismatch. Expected {v}, got {result.get(k)}")
