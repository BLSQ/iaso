...
from iaso.models import StockRulesVersionsStatus
from iaso.models.stocks import StockImpacts, StockRulesVersion
...
class StockKeepingUnitAPITestCase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.user_with_rights, cls.anon_user, cls.user_without_rights = cls.create_base_users(
            account_1, [core_permissions._STOCK_MANAGEMENT]
        )
...
class StockItemRuleAPITestCase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.version_2 = version_2 = m.StockRulesVersion.objects.create(
            name="Finalized", account=account_1, status=StockRulesVersionsStatus.PUBLISHED
        )
    ...
    def test_patch_rule(self):
        ...
        response = self.client.patch(
            f"{RULE_ITEM_URL}{self.stock_item_rule_1.pk}/", data={"impact": StockImpacts.SUBTRACT}
        )
        self.assertJSONResponse(response, 200)

        response = self.client.get(f"{RULE_ITEM_URL}{self.stock_item_rule_1.pk}/")
        self.assertEqual(response.json()["impact"], StockImpacts.SUBTRACT)
    ...
    def test_patch_for_finalized_version(self):
        self.client.force_authenticate(self.user_with_rights)
        response = self.client.patch(
            f"{RULE_ITEM_URL}{self.stock_item_rule_2.pk}/", data={"impact": StockImpacts.SUBTRACT}
        )
        self.assertJSONResponse(response, 403)
...
class StockRulesVersionAPITestCase(APITestCase):
    ...
    def test_change_status(self):
        ...
        version = m.StockRulesVersion.objects.create(
            name="Published", account=self.account_1, status=StockRulesVersionsStatus.PUBLISHED
        )
        self.assertEqual(1, StockRulesVersion.objects.filter(status=StockRulesVersionsStatus.PUBLISHED).count())
        self.assertEqual(version, StockRulesVersion.objects.get(status=StockRulesVersionsStatus.PUBLISHED))

        response = self.client.patch(
            f"{RULES_VERSION_URL}{self.version_1.pk}/", data={"status": StockRulesVersionsStatus.PUBLISHED}
        )
        self.assertJSONResponse(response, 200)

        self.assertEqual(1, StockRulesVersion.objects.filter(status=StockRulesVersionsStatus.PUBLISHED).count())
        response = self.client.get(f"{RULES_VERSION_URL}{self.version_1.pk}/")
        self.assertEqual(response.json()["status"], StockRulesVersionsStatus.PUBLISHED)
        self.assertEqual(m.StockRulesVersion.objects.get(pk=version.pk).status, StockRulesVersionsStatus.UNPUBLISHED)
