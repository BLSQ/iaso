...
class TargetsViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if user_ids is not None:
            queryset = queryset.filter(
                Q(gps_import__user_id__in=user_ids.split(","))
                | Q(api_import__user_id__in=user_ids.split(","))
            )
        ...
        if csv_format is None and xlsx_format is None:
            ...
            def get_row(target, **kwargs):
                ...
                return [
                    ...
                    tdict.get("gps_import")
                    .get("created_at")
                    .strftime("%Y-%m-%d %H:%M:%S")
                    if tdict.get("gps_import")
                    else "",
                    tdict.get("api_import")
                    .get("created_at")
                    .strftime("%Y-%m-%d %H:%M:%S")
                    if tdict.get("api_import")
                    else "",
                    tdict.get("api_import").get("user")
                    if tdict.get("api_import")
                    else "",
                    tdict.get("river"),
                    tdict.get("village"),
    ...
    def retrieve(self, request, pk=None):
        ...
        province = (
            Province.objects.filter(geom__contains=target.location)[0]
            if Province.objects.filter(geom__contains=target.location).count() > 0
            else None
        )
        zone = (
            ZS.objects.filter(geom__contains=target.location)[0]
            if ZS.objects.filter(geom__contains=target.location).count() > 0
            else None
        )
        area = (
            AS.objects.filter(geom__contains=target.location)[0]
            if AS.objects.filter(geom__contains=target.location).count() > 0
            else None
        )
        is_authorized = (
