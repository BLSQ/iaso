...
from iaso.models import EntityType, Task, TaskLog
...
def extract_programme(data):
    ...
    for field in ("programme", "_programme"):
        val = data.get(field)
        if val is not None:
    ...
    prog2 = data.get("program_two")
    if prog2 is not None and prog2 != "NONE":
        return prog2
    ...
    if data.get("program") == "NONE" and data.get("physiology_status") not in (
        None,
        "",
    ):
        return "TSFP"
...
def _extract_rations(data):
    ...
    # Generic ration type (ration_type field or others not handled above)
        ...
        if ration_type in ("csb", "csb1", "csb2"):
            quantity = _safe_float(data.get("_csb_packets"), 0)
...
class Under5SSDV2:
    ...
    * Journey boundaries are determined by admission forms, with
    ...
    def run(self, entity_type_code=None, updated_entity_ids=None, task_name="etl_ssd_v2"):
        ...
        submissions = self._retrieve_submissions(code, updated_entity_ids)
        ...
        for entity_id, entity_submissions in groupby(submissions, key=itemgetter("entity_id")):
            ...
            result = self._process_entity(entity_id, entity_subs, account, existing_entity_ids)
            if result is None:
                skipped_count += 1
                continue
        ...
        logger.info(
            f"Processed {entity_count} entities total ({skipped_count} skipped). "
    ...
    @staticmethod
    def _get_account(entity_type_code):
        entity_type = EntityType.objects.select_related("account").filter(code=entity_type_code).first()
        return entity_type.account

    @staticmethod
    def _retrieve_submissions(entity_type_code, entity_ids=None):
    ...
    def _process_entity(self, entity_id, submissions, account, existing_entity_ids):
        ...
        for journey_subs in journey_groups:
            result = self._process_journey(journey_subs, beneficiary, transfer_info)
    ...
    def _extract_beneficiary_info(submissions):
        ...
        info = {"gender": None, "birth_date": None, "guidelines": None}
    ...
    def _is_valid_beneficiary(info):
        return info["gender"] in ("Male", "Female") and info["birth_date"] is not None
    ...
    def _process_journey(self, submissions, beneficiary, transfer_info=None):
