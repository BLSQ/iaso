...
def migrate_permissions(apps, schema_editor):
    ...
    # When a Django migration creates a new model, Django also creates a new ContentType for that model.
    # Each class that handles permissions in Django is therefore linked to a ContentType.
    # When you run manage.py migrate, Django will run migration files and then send a 'post_migrate' signal,
    # Permissions and ContentTypes are created by the post_migrate signal (see Django source code).
    # So, if you create a new permission model and need to create a second migration that interacts with its data,
    # you'll probably encounter issues during deployment because the ContentType won't be ready in the 2nd migration file.
    # The best way to fix this is to execute create_permissions, which is normally called by 'post_migrate'
    # Thanks @kemar for the research - https://github.com/BLSQ/iaso/pull/2303/files#r2236351050
    app_config = global_apps.get_app_config("iaso")
    ...
    for old_codename, new_codename in permission_mapping.items():
        ...
        try:
            ...
            new_perm = Permission.objects.get(codename=new_codename, content_type=new_ct)
