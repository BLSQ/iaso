...
def gpx_upload(request: HttpRequest) -> HttpResponse:
    ...
    for gpx_file in request.FILES.getlist("gpx"):
        gpx_import = gpximport(
            gpx_file.name, gpx_file.file, request.user
    ...
    if len(imports) > 0:
        return JsonResponse({"imports": imports}, status=status.HTTP_201_CREATED)
    else:
        return JsonResponse(
            {"Error": "No gpx file provided"}, status=status.HTTP_400_BAD_REQUEST
        )
