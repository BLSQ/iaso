    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        if restricted:
            ...
            self.fields["ZS"] = forms.ChoiceField(
                choices=[none_choice]
                + [(row["id"], row["name"]) for row in locations_choices_zs],
                widget=forms.Select(attrs=select_attrs),
                required=False,
            )
        ...
        self.fields["AS"] = forms.ChoiceField(
            choices=[none_choice]
            + [(row["id"], row["name"]) for row in locations_choices_as],
            widget=forms.Select(attrs=select_attrs),
            required=False,
        )
        ...
        self.fields["village"] = forms.ChoiceField(
            choices=[none_choice]
            + [(row["id"], row["name"]) for row in locations_choices_village],
            widget=forms.Select(attrs=select_attrs),
            required=False,
        )
        ...
        self.fields["date_from"] = forms.DateField(
            widget=DateInput(attrs=date_attrs), required=False,
        )
...
def filter_and_create_form(
    ...
    location_choices_village = (
        cc_locations_list.values_list("normalized_village_id", flat=True)
        .distinct("normalized_village_id")
        .order_by("normalized_village_id")[:100]
    )
    location_choices_as = (
        cc_locations_list.values_list("normalized_AS_id", flat=True)
        .distinct("normalized_AS_id")
        .order_by("normalized_AS_id")[:100]
    )
    location_choices_zs = (
        cc_locations_list.values_list("normalized_AS__ZS_id", flat=True)
        .distinct("normalized_AS__ZS_id")
        .order_by("normalized_AS__ZS_id")
    )

    from hat.geo.models import (
        Village,
        ZS,
        AS,
    )  # had to import here for it to work, no idea why https://docs.python.org/3/faq/programming.html#what-are-the-best-practices-for-using-import-in-a-module

    location_choices_village_name = (
        Village.objects.filter(id__in=location_choices_village)
        .values("id", "name")
        .order_by("name")
    )
    location_choices_as_name = (
        AS.objects.filter(id__in=location_choices_as)
        .values("id", "name")
        .order_by("name")
    )
    location_choices_zs_name = (
        ZS.objects.filter(id__in=location_choices_zs)
        .values("id", "name")
        .order_by("name")
    )
...
class CaseForm(ModelForm):
    ...
    try:
        ...
        AS_choices = (
            AS.objects.order_by("name")
            .values_list("name", "name")
            .distinct()
            .__repr__()
        )
        ZS_choices = (
            ZS.objects.order_by("name")
            .values_list("name", "name")
            .distinct()
            .__repr__()
        )
        ...
        TREATMENT_CENTER_choices = (
            Case.objects.order_by("treatment_center")
            .values_list("treatment_center", "treatment_center")
            .distinct()
            .__repr__()
        )
    except:
