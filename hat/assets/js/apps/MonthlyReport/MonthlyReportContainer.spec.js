...
const checkAction = (calls, actionType) =>
    calls.some(args => args[0] && args[0].type === actionType);
...
describe('MonthlyReportContainer Loading Data', () => {
    ...
    it('loads data on initialization', () => {
        renderWithStore(reduxStore, <MonthlyReportContainer {...defaultProps} />);
        assert(nockScope.isDone(), 'The urls have been requested');
    });

    it('loads data when the filter params change', () => {
        const node = document.createElement('div');
        renderWithStore(reduxStore, <MonthlyReportContainer {...defaultProps} />, node);
        assert(nockScope.isDone(), 'The urls have been requested');
        ...
        renderWithStore(reduxStore, <MonthlyReportContainer {...props2} />, node);
        ...
        renderWithStore(reduxStore, <MonthlyReportContainer {...props3} />, node);
    ...
    it('does not redirect on "all"', (done) => {
        ...
        renderWithStore(reduxStore, <MonthlyReportContainer {...props} />);
        setTimeout(() => {
