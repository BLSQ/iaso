...
CALENDAR_URL = "/api/polio/v2/calendar/"

NOW = datetime.datetime(2021, 1, 1, 0, 0, 0, tzinfo=datetime.timezone.utc)


@time_machine.travel(NOW, tick=False)
class CampaignCalendarV2TestCase(APITestCase, PolioTestCaseMixin):
    @classmethod
    ...
    def test_response_shape(self):
        ...
        self.assertEqual(campaign["general_status"], "Round 1 started")
    ...
    def test_response_has_rounds(self):
        self.client.force_authenticate(self.user)
        response = self.client.get(CALENDAR_URL, format="json")
        response = self.assertJSONResponse(response, 200)
        self.assertEqual(len(response), 1)
