...
export default function iasoApp(element, baseUrl) {
    let routes = routeConfigs.map(routeConfig => (
        <Route
            ...
            component={props => <ProtectedRoute {...props} permission={routeConfig.permission} component={routeConfig.component(props)} isRootUrl={routeConfig.isRootUrl} />}
        />
    ));
    ...
    routes = routes.concat([
        ...
        <Redirect
            ...
            to={`${baseUrls.orgUnits}/locationLimit/${locationLimitMax}/order/id/pageSize/50/page/1/searchTabIndex/0/searches/[{"validation_status":"all", "color":"${getChipColors(0).replace(
                '#',
                '',
            )}"}]`}
        />,
