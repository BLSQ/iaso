...
class BulkCreateCsvTestCase(APITestCase):
    ...
    def test_audit_log_on_save(self):
        ...
        user_names = [log["new_value"][0]["user"]["username"] for log in logs_for_new_users]
        ...
        self.assertEquals(new_value["user_roles"][0]["name"], "manager")
        self.assertEquals(len(new_value["projects"]), 1)
        self.assertEquals(new_value["projects"][0]["name"], "Project name")
