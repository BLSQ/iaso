...
APPLICABLE_URLS = ("/api/validation-workflows/",)


def selective_camelize_serializer_fields(result, generator, request, public):
    ...
    def has_middleware_installed():
        ...
        return any(
            isinstance(m, type) and issubclass(m, CamelCaseMiddleWare) for m in map(import_string, settings.MIDDLEWARE)
        )
    ...
    for (_, component_type), component in generator.registry._components.items():
        if component_type == "schemas":
            # here we just apply this to the related camelCase viewsets
            if CamelCaseJSONRenderer in getattr(component.object.context.get("view", None), "renderer_classes", []):
                camelize_component(component.schema)

    if has_middleware_installed():
        for key, url_schema in result["paths"].items():
            if key.startswith(APPLICABLE_URLS):
