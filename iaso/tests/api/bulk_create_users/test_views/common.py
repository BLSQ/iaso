...
from iaso.models import BulkCreateUserFile, Profile, Team, UserRole
...
class BulkCreateBaseAPITestCaseMixin:
    @classmethod
...
class BulkCreateAPITestCase(BulkCreateBaseAPITestCaseMixin):
    """
    Purpose of this base test and mixin above is to centralize all tests into one place and just change formats dynamically with subclasses (xlsx/csv)
    ...
    def read_content_from_fixture_file(self, fixture_name):
        file_content = None
        with open(f"iaso/tests/fixtures/bulk_create_users/{fixture_name}.csv") as file_users:
            file_content = file_users.read()
        return file_content

    def generate_file_from_fixture(self, fixture_name, context=None, with_context=True):
        if with_context:
            file_content = self.create_from_fixture(
                fixture_name=fixture_name,
                context=context,
            )
        else:
            file_content = self.read_content_from_fixture_file(fixture_name)

        return self.generate_simple_uploaded_file(fixture_name, file_content)
    ...
    def test_upload_valid_file(self):
        ...
        test_file = self.generate_file_from_fixture("test_user_bulk_create_valid", context)
        ...
        file_upload = m.BulkCreateUserFile.objects.first()
        ...
        expected_file_name = f"{self.account1.short_sanitized_name}_{self.account1.id}/bulk_create_user_file/{file_upload.created_at.strftime('%Y_%m')}/test_user_bulk_create_valid.{self._file_extension}"
        self.assertEqual(file_upload.file.name, expected_file_name)
        ...
        self.assertTrue(len(list(self.get_rows_from_file(file_upload.file))) > 0)

        for row in self.get_rows_from_file(file_upload.file):
    ...
    def test_num_queries(self):
        ...
        test_file = self.generate_file_from_fixture(
            fixture_name="test_user_bulk_create_valid",
            context=context,
        )
    ...
    def test_num_queries_with_perms(self):
        self.client.force_authenticate(self.yoda)
        self.source.projects.set([self.project])
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_valid_with_perm", with_context=False)

        with self.assertNumQueries(47):
            response = self.client.post(f"{BASE_URL}", {"file": file}, format="multipart")
        self.assertJSONResponse(response, status.HTTP_201_CREATED)

    def test_upload_valid_file_with_perms(self):
        self.client.force_authenticate(self.yoda)
        self.source.projects.set([self.project])
        ...
        pollux = get_user_model().objects.get(username="pollux")
    ...
    def test_upload_invalid_mail(self):
        self.client.force_authenticate(self.yoda)
        self.source.projects.set([self.project])

        file = self.generate_file_from_fixture("test_user_bulk_create_invalid_mail", with_context=False)
        ...
        self.assertIn("email", validation_errors[0]["details"])
    ...
    def test_create_user_with_empty_first_name_and_last_name(self):
        ...
        test_file = self.generate_file_from_fixture("test_user_bulk_create_empty_names", context)

        response = self.client.post(f"{BASE_URL}", {"file": test_file}, format="multipart")
        self.assertJSONResponse(response, status.HTTP_201_CREATED)

        users = get_user_model().objects.all()
        self.assertEqual(users.count(), initial_user_count + 1)
    ...
    def test_create_user_with_project_by_id(self):
        ...
        test_file = self.generate_file_from_fixture("test_user_bulk_create_valid_with_project_ids", context)
        response = self.client.post(f"{BASE_URL}", {"file": test_file}, format="multipart")
        res_json = self.assertJSONResponse(response, status.HTTP_201_CREATED)
        self.assertEqual(res_json, {})

        user = get_user_model().objects.get(username="projectid_user")
    ...
    def test_upload_invalid_language(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_invalid_language", with_context=False)
        ...
        self.assertEqual(len(res_data["file_content"]), 1)
        self.assertIn("profile_language", res_data["file_content"][0]["details"])
        self.assertIn("invalid_lang", res_data["file_content"][0]["details"]["profile_language"][0])
    ...
    def test_upload_invalid_permission_name(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_invalid_permission", with_context=False)
        ...
        validation_errors = res_data["file_content"]
        self.assertEqual(len(validation_errors), 1)
        self.assertIn("permissions", validation_errors[0]["details"])
        self.assertIn("iaso_fake_permission", validation_errors[0]["details"]["permissions"][0])
    ...
    def test_default_fields_three_scenarios(self):
        ...
        Team.objects.create(name="Test Team", project=self.project, manager=self.yoda)
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_bulk_configuration", with_context=False)
        ...
        # Verify default options are stored in BulkCreateUserFile (now M2M fields)
        csv_file_record = BulkCreateUserFile.objects.last()
        ...
        all_bulk_user = get_user_model().objects.get(username="all_bulk_user")
        ...
        no_bulk_user = get_user_model().objects.get(username="no_bulk_user")
        ...
        mixed_user = get_user_model().objects.get(username="mixed_user")
    ...
    @patch("iaso.api.bulk_create_users.serializers.send_bulk_email_invitations")
    def test_email_invitations_mixed_scenario_with_fixture(self, mock_send_emails):
        """Test email invitations in mixed scenario."""
        self.client.force_authenticate(self.yoda)
        self.source.projects.set([self.project])

        file = self.generate_file_from_fixture("test_user_bulk_create_mixed_email", with_context=False)
        ...
        with_password = get_user_model().objects.get(username="with_password")
        no_password1 = get_user_model().objects.get(username="no_password1")
        no_password2 = get_user_model().objects.get(username="no_password2")
        no_email = get_user_model().objects.get(username="no_email")
    ...
    def test_email_invitations_sent_for_users_without_password(self, mock_send_emails):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_email_no_password", with_context=False)
        ...
        invite1 = get_user_model().objects.get(username="invite1")
        invite2 = get_user_model().objects.get(username="invite2")
    ...
    def test_bulk_create_user_cannot_assign_team_from_another_account(self):
        ...
        han_solo = get_user_model().objects.get(username="han solo")
        ...
        test_file = self.generate_file_from_fixture("test_user_bulk_create_valid_with_multiple_teams", context)
        ...
        validation_errors = res_data["file_content"]
        self.assertEqual(len(validation_errors), 1)
        self.assertIn("teams", validation_errors[0]["details"])
        self.assertIn(secret_team_name, validation_errors[0]["details"]["teams"][0])
    ...
    def test_bulk_create_user_fails_on_invalid_team(self):
        ...
        self.assertEqual(len(validation_errors), 1)
        self.assertIn("teams", validation_errors[0]["details"])
        self.assertIn(invalid_team_name, validation_errors[0]["details"]["teams"][0])
    ...
    def test_bulk_create_user_with_multiple_teams(self):
        ...
        user = get_user_model().objects.get(username="rsfg")
    ...
    def test_audit_log_on_save(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_all_fields", with_context=False)
    ...
    def test_should_create_user_with_the_correct_org_unit_from_source_ref(self):
        ...
        file = self.generate_file_from_fixture("test_should_create_user_with_the_correct_org_unit_from_source_ref", {})

        response = self.client.post(f"{BASE_URL}", {"file": file}, format="multipart")
        self.assertJSONResponse(response, status.HTTP_201_CREATED)

        new_user = get_user_model().objects.get(email="john@foo.com")
    ...
    def test_create_user_with_project_restrictions(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_managed_geo_limit", with_context=False)
    ...
    def test_upload_without_mail_must_work(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_no_mail", with_context=False)
    ...
    def test_upload_invalid_orgunit_id(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_invalid_orgunit", with_context=False)
        ...
        self.assertIn("orgunit", validation_errors[0]["details"])
        self.assertIn("99998", validation_errors[0]["details"]["orgunit"][0])

        # Check second and third errors - invalid source_ref
        self.assertEqual(int(validation_errors[1]["row"]), 2)
        self.assertIn("orgunit__source_ref", validation_errors[1]["details"])
        self.assertEqual(int(validation_errors[2]["row"]), 3)
        self.assertIn("orgunit__source_ref", validation_errors[2]["details"])
    ...
    def test_upload_user_already_exists(self):
        ...
        username_error = next((e for e in validation_errors if "username" in e.get("details", {})), None)
        self.assertIsNotNone(username_error)
        self.assertEqual(username_error["details"]["username"], ["This field must be unique."])
    ...
    def test_create_user_with_projects(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_valid_with_projects", with_context=False)
    ...
    def test_upload_csv_with_missing_column(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_missing_columns", with_context=False)

        response = self.client.post(f"{BASE_URL}", {"file": file}, format="multipart")

        res_data = self.assertJSONResponse(response, status.HTTP_400_BAD_REQUEST)
        self.assertHasError(res_data, "file_content", {"general": "Missing required column(s): permissions"})
    ...
    def test_create_user_with_roles(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_valid_with_roles", with_context=False)
    ...
    def test_can_create_user_with_ou_that_are_child_of_ou(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_user_access_to_child_ou", with_context=False)

        response = self.client.post(f"{BASE_URL}", {"file": file}, format="multipart")

        self.assertJSONResponse(response, status.HTTP_201_CREATED)
        self.assertEqual(get_user_model().objects.filter(username="jan").exists(), True)

        created_user = get_user_model().objects.get(username="jan")
    ...
    def test_upload_duplicate_ou_names(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_user_duplicate_ou_names", with_context=False)
    ...
    def test_cant_create_user_without_ou_profile(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_creator_no_access_to_ou", with_context=False)
        ...
        self.assertHasError(res_data, "file_content")
        validation_errors = res_data["file_content"]
        orgunit_error = next((e for e in validation_errors if "orgunit" in e.get("details", {})), None)
        self.assertIsNotNone(orgunit_error)
        # The error should indicate access issue or invalid org unit
        self.assertIn("10244", orgunit_error["details"]["orgunit"][0])
    ...
    def test_users_profiles_have_right_ou(self):
        ...
        broly_ou = Profile.objects.get(user=get_user_model().objects.get(username="broly").id).org_units.all()
        ou_list = []

        for ou in broly_ou:
            ou_list.append(ou.id)

        ferdinand_ou = Profile.objects.get(user=get_user_model().objects.get(username="ferdinand").id).org_units.all()
        ou_f_list = []
    ...
    def test_upload_invalid_orgunit_name(self):
        ...
        file = self.generate_file_from_fixture("test_user_bulk_create_invalid_ou_name", with_context=False)
        ...
        orgunit_error = next((e for e in validation_errors if "orgunit" in e.get("details", {})), None)
        self.assertIsNotNone(orgunit_error)
        self.assertIn("Bazarre", orgunit_error["details"]["orgunit"][0])
    ...
    def test_cant_create_user_not_org_unit_in_the_pyramid_with_managed_geo_limit_permission(self):
        ...
        for ve in validation_errors:
            self.assertEqual(ve["details"], {"orgunit": ["Object with pk=1112 does not exist."]})
