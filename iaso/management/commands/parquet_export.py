...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if mode == "pyramid":
            ...
            export_django_query_to_parquet_via_duckdb(
                self.build_orgunit_queryset(m.OrgUnit.objects.filter(version_id=version_id)), "./media/pyramid.parquet"
            )
        ...
        if mode == "submissions":
            ...
            export_django_query_to_parquet_via_duckdb(
                self.build_submissions_queryset(m.Instance.objects, form_id), "./media/submissions.parquet"
            )

    def build_submissions_queryset(self, qs, form_id):
    ...
    def build_orgunit_queryset(self, qs):
        ...
        max_ancestor_level = qs.aggregate(
            max_level=Max(RawSQL("array_length(string_to_array(path::text, '.'), 1)", []))
        )["max_level"]
