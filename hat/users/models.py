...
def is_authorized_user(user, province_id, zone_id, area_id):
    ...
    is_authorized = (
        len(user_as_ids) == 0 and len(user_zs_ids) == 0 and len(user_province_ids) == 0
    )
...
class Coordination(models.Model):
    ...
    def as_dict(self):
        ...
        if user.profile.province_scope.count() != 0:
            zs_query = zs_query.filter(
                province__id__in=user.profile.province_scope.all().values_list(
                    "pk", flat=True
                )
            ).distinct()
        if user.profile.ZS_scope.count() != 0:
            zs_query = zs_query.filter(
                id__in=user.profile.ZS_scope.all().values_list("pk", flat=True)
            ).distinct()
        return {
            "name": self.name,
...
class Team(models.Model):
    ...
    def as_dict(self, planning_id=None):
        ...
        if planning_id is not None:
            ...
            res["AS"] = [
                taz.area.as_dict()
                for taz in TeamActionZone.objects.filter(
                    planning_id=planning_id, team_id=self.id
                ).select_related("area")
            ]
        return res

    def as_dict_without_as(self):
        return {"name": self.name, "id": self.id, "capacity": self.capacity}
...
class Profile(models.Model):
    ...
    userType = models.ForeignKey(
        UserType, null=True, blank=True, on_delete=models.SET_NULL
    )
