...
@task_decorator(task_name="export_entities_to_records")
def export_entities_to_records(task=None):
    # Handle admission forms
    task.report_progress_and_stop_if_killed(
        progress_value=1,
        end_value=4,
        progress_message="Processing instances starting",
    )
    ...
    task.report_progress_and_stop_if_killed(
        progress_value=2,
        end_value=4,
        progress_message="Instances loaded. Will now process admission instances.",
    )
    # Process admission instances
    process_instances(admission_instances, "admission")
    task.report_progress_and_stop_if_killed(
        progress_value=3,
        end_value=4,
        progress_message="Instances for admissions processed. Will now process follow-up instances.",
    )
    # Process follow-up instances
...
def process_instances(instances, form_type):
