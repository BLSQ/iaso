...
class OrgUnitTypesV2UpdateTestCase(SwaggerTestCaseMixin, APITestCase):
    @classmethod
    ...
    def assertValidBodyData(self, data):
        self.assertResponseCompliantToSwagger(data, "OrgUnitTypeUpdateRequest")

    def assertValidResponseData(self, data):
        self.assertResponseCompliantToSwagger(data, "OrgUnitTypeUpdate")

    def test_permissions(self):
        res = self.client.patch(reverse("orgunittypes_v2-detail", kwargs={"pk": self.org_unit_type_1.id}))
    ...
    def test_num_queries(self):
        ...
        with self.assertNumQueries(16):
            res = self.client.patch(
                reverse("orgunittypes_v2-detail", kwargs={"pk": self.org_unit_type_1.id}), data=data
            )
        self.assertEqual(res.status_code, status.HTTP_200_OK)

    def test_org_unit_type_update_invalid(self):
        """POST /orgunittypes/ without project ids: invalid"""

        self.client.force_authenticate(self.jane)
        response = self.client.patch(
            reverse("orgunittypes_v2-detail", kwargs={"pk": self.org_unit_type_1.id}),
    ...
    def test_org_unit_type_update_invalid_wrong_project(self):
        """POST /orgunittypes/ without project ids: invalid"""
    ...
    def test_org_unit_type_update_with_not_existing_reference_form_ok(self):
    ...
    def test_org_unit_type_update_with_reference_form_ok(self):
        """POST /orgunittypes/ with auth: 201 OK"""
    ...
    def test_org_unit_type_update_with_reference_form_wrong_project(self):
    ...
    def test_org_unit_type_update_with_sub_unit_types_ok(self):
        """POST /orgunittypes/ with auth: 201 OK"""
        data = {
            "name": "Bimbam",
            "short_name": "Bi",
    ...
    def test_org_unit_type_update_with_reference_form_id_ok(self):
        ...
        self.assertEqual(
            list(self.org_unit_type_1.reference_forms.values_list("id", flat=True)), [self.reference_form_update.id]
        )
