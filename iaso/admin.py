...
class UserAdmin(AuthUserAdmin):
    ...
    def edit_multi_account_user(self, request, user_id):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                if not user.tenant_users.exists():  # User is not yet multi-account
                    # Move existing account to tenant_user structure
                    existing_profile = user.iaso_profile
                    existing_account = existing_profile.account
                    new_account_user = User.objects.create(
                        username=f"{user.username}_account_{str(existing_account.pk)}",
                ...
                for account in form.cleaned_data["accounts"]:
                    with transaction.atomic():
                        new_account_user = User.objects.create(
                            username=f"{user.username}_account_{str(account.pk)}",
                            first_name=user.first_name,
                            last_name=user.last_name,
                            email=user.email,
                            is_superuser=user.is_superuser,
                ...
                # Implement your logic to link the user to the selected accounts
                # Example: user.iaso_profile.account.set(accounts)
                messages.success(request, f"User {user.username} has been linked to selected accounts.")
        ...
        return render(request, "admin/edit_multi_account_user.html", {"form": form, "user": user})
