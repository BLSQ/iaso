"""WFP ETL - Under 5 children and PBWG, South Sudan
...
from plugins.wfp.models import Beneficiary, Dhis2SyncResults, Journey, MonthlyStatistics, ScreeningData, Step, Visit
...
class ETL:
    ...
    * Journey boundaries are determined by admission forms, with
    ...
    def delete_beneficiaries(self, account_id):
        ...
        if deleted_count > 0:
            print("Cleanup complete %d items removed", deleted_count)
            print("Deleted %d Beneficiaries", deleted.get("wfp.Beneficiary", 0))
            print("Deleted %d Journeys", deleted.get("wfp.Journey", 0))
            print("Deleted %d Visits", deleted.get("wfp.Visit", 0))
            print("Deleted %d Steps", deleted.get("wfp.Step", 0))
        else:
    ...
    def _retrieve_aggregated_journeys_data(account, program_type, org_units, page_size=5000, page_number=1):
        ...
        queryset = (
            ...
            .annotate(
                ...
                community_health_worker_muac_above_23=Count(
                    "journey__beneficiary_id",
                    filter=Q(
                        entry_point="community_health_worker", journey__programme_type="PLW", muac_numeric__gte=23
                    ),
                    distinct=True,
                ),
    ...
    def group_data_to_push_to_dhis2(account, org_unit_ids, page_size=5000, page_number=1):
        ...
        screening_data = ETL.screening_data(account, current_page_org_units)
        ...
        # Loop on the monthly statistic in order to merge screening data and referals
        # from community worker cases with the monthly statistics one
        merged_data = []
