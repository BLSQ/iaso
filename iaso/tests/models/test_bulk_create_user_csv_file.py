...
class BulkCreateUserFileTestCase(FileUploadToTestCase):
    ...
    def test_upload_to_happy_path(self):
        ...
        with open(self.FILE_PATH) as csv_file:
            bulk_create = BulkCreateUserFile.objects.create(
                created_by=self.user_1,
                account=self.account_1,
                file=UploadedFile(csv_file),
            )

        expected_file_name = f"{self.account_1.short_sanitized_name}_{self.account_1.id}/bulk_create_user_file/{bulk_create.created_at.strftime('%Y_%m')}/{self.FILE_NAME}"
    ...
    def test_upload_to_invalid_account_name(self):
        ...
        expected_file_name = f"invalid_name_{self.account_2.id}/bulk_create_user_file/{bulk_create.created_at.strftime('%Y_%m')}/{self.FILE_NAME}"
        self.assertEqual(bulk_create.file.name, expected_file_name)
