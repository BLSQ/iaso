...
describe('formatRoundDateRange', () => {
    ...
    it('joins start and end with the to label', () => {
        expect(formatRoundDateRange('2024-03-15', '2024-03-20', toLabel)).toBe(
            '15/03/2024 to 20/03/2024',
        );
    });

    it('returns only the start when the end is missing', () => {
        expect(formatRoundDateRange('2024-03-15', null, toLabel)).toBe(
            '15/03/2024',
        );
    });

    it('returns only the end when the start is missing', () => {
        expect(formatRoundDateRange(undefined, '2024-03-20', toLabel)).toBe(
            '20/03/2024',
        );
    });

    it('returns only the valid side when the other date is invalid', () => {
        expect(formatRoundDateRange('not-a-date', '2024-03-20', toLabel)).toBe(
            '20/03/2024',
        );
        expect(formatRoundDateRange('2024-03-15', 'not-a-date', toLabel)).toBe(
            '15/03/2024',
        );
    });
