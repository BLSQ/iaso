...
class VillageTest(APITestCase):
    ...
    def test_list_village(self):
        self.assertTrue(
            self.client.login(username="supervisor", password="supervisorsupervisor")
        )
    ...
    def test_list_village_limited(self):
        self.assertTrue(
            self.client.login(
                username="supervisor-mosango", password="supervisorsupervisor"
            )
        )
        url = reverse("village-list")
        response = self.client.get(url, format="json")
    ...
    def test_list_village_muluma_limited(self):
        self.assertTrue(
            self.client.login(
                username="supervisor-muluma", password="supervisorsupervisor"
            )
        )
        url = reverse("village-list")
        response = self.client.get(url, format="json")
    ...
    def test_list_prov_kwilu_limited(self):
        self.assertTrue(
            self.client.login(
                username="supervisor-kwilu", password="supervisorsupervisor"
            )
        )
        url = reverse("village-list")
        response = self.client.get(url, format="json")
    ...
    def test_retrieve_prov_kwilu_limited_boko(self):
        ...
        url = reverse(
            "village-detail", args=[2111]
        )  # Boko in Kwango, should not be visible
        response = self.client.get(url, format="json")
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
