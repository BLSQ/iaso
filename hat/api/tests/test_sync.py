...
class SyncTests(APITestCase):
    ...
    def test_get_credentials(self, verify_token_function):
        ...
        self.assertEqual(creds['user_name'], 'test_karl@ehealthnigeria.org')
        self.assertTrue(creds['password'], msg='provides a password')
        self.assertTrue(creds['db_name'], msg='provides a db name')
    ...
    def test_update_credentials(self, verify_token_function):
        ...
        response2 = self.client.post(url, {'idToken': 'Long JWT Token'}, format='json')
        creds2 = response2.data

        self.assertEqual(response2.status_code, 201)

        self.assertEqual(creds['user_name'], creds2['user_name'])
        self.assertEqual(creds['db_name'], creds2['db_name'])
    ...
    def test_no_token(self, valid_token_function):
        '''what happens if nothing is posted?'''
        url = reverse('api:sync')
        response = self.client.post(url, {'idToken': ''}, format='json')
    ...
    def test_invalid_token(self, valid_token_function):
        ...
        url = reverse('api:sync')
        response = self.client.post(url, {'idToken': 'invalid'}, format='json')
