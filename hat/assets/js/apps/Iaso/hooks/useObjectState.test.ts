...
describe("useObjectState's recursiveReducer", () => {
    ...
    it('updates only keys passed in argument', () => {
        ...
        expect(result.red.firstName).toBe('Albert');
        expect(result.red.name).toBe('Shiba');
        expect(result.red.powerSet.weapon).toBe('Rekka Daizanto');
        expect(result.blue.firstName).toBe('Ryuunosuke');
    });
