...
L.Control.ZoomBar = L.Control.Zoom.extend({
  ...
  onAdd: function (map) {
    ...
    var container = L.DomUtil.create('div', className + ' leaflet-bar hide-on-print')
    ...
    if (options.fitToBounds) {
      this._fitToBoundsButton = this._createButton('', options.fitToBoundsTitle,
   ...
   ****************************************************************************/
  _zoomBox: function () {
    this._zoomBoxActive ? this._zoomBoxStateOff() : this._zoomBoxStateOn()
  },
  _zoomBoxHTML: function () {
    return ''
  },
  _zoomBoxStateOn: function () {
  ...
  _zoomBoxHandleMouseDown: function (event) {
